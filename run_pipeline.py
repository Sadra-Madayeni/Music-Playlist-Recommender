...
def run_import(scripts_dir):
    if not os.path.exists("database/dataset.db"):
        ...
        subprocess.run([sys.executable, os.path.join(scripts_dir, "import_to_db.py")], check=True)
    else:
...
def run_preprocess(scripts_dir):
    ...
    subprocess.run([sys.executable, os.path.join(scripts_dir, "preprocess.py")], check=True)

@task(name="Feature Engineering")
def run_feature_engineering(scripts_dir):
    print("Running feature_engineering.py...")
    subprocess.run([sys.executable, os.path.join(scripts_dir, "feature_engineering.py")], check=True)
...
def run_model_training(scripts_dir):
    ...
    subprocess.run([sys.executable, os.path.join(scripts_dir, "recommender_model.py")], check=True)

@task(name="Generate Playlists")
def run_playlist_generator(scripts_dir):
    print("Running playlist_generator.py...")
    subprocess.run([sys.executable, os.path.join(scripts_dir, "playlist_generator.py")], check=True)
...
def main_pipeline():
    ...
    # Define dependencies explicitly if needed, but sequential call works in standard Prefect flow
