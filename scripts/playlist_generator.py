...
def generate_mixes(query_track_ids, dataset_filepath, model_filepath, scaler_filepath):
    ...
    for _, row in query_df.iterrows():
        output_lines.append(
            f"- {row ['track_name']} by {row ['artists']} (Genre: {row ['track_genre']})"
        )
    ...
    model = SessionRecommenderLSTM(
        input_dim=len(feature_cols), hidden_dim=32, output_dim=len(feature_cols)
    )
    ...
    print(
        f"Detected {len (main_genres )} main genres (>= {min_tracks } tracks). Setting K={best_k }."
    )
    ...
    if pca_result is not None:
        ...
        plt.legend(
            bbox_to_anchor=(1.05, 1), loc="upper left", title="Clusters & Genres"
        )
        plt.tight_layout()
        plot_path = os.path.join(
            os.path.dirname(dataset_filepath), "query_clusters.png"
        )
    ...
    merged = df.merge(
        query_df[["track_name", "artists"]].drop_duplicates(),
    ...
    candidates["for_you_score"] = (
        candidates["similarity_score"]
        + candidates["artist_bonus"]
        + candidates["genre_bonus"]
    )
    for_you = (
        candidates.sort_values(by="for_you_score", ascending=False)
    ...
    for _, row in for_you.iterrows():
        output_lines.append(
            f"  - {row ['track_name']} by {row ['artists']} (Genre: {row ['track_genre']}, Score: {row ['for_you_score']:.2f}, Artist Bonus: {row ['artist_bonus']:.3f})"
        )
    output_lines.append("")
    ...
    for c_id in sorted(list(unique_clusters)):
        ...
        if "similar_genre_1" in df.columns:
            dom_row = cluster_tracks[
                cluster_tracks["track_genre"] == dominant_genre
            ].iloc[0]
            total_cluster_tracks = len(cluster_tracks)
            local_genre_counts = cluster_tracks["track_genre"].value_counts().to_dict()
            cluster_percentages = {
                g: count / total_cluster_tracks
                for g, count in local_genre_counts.items()
            }
            allowed = cluster_percentages.copy()
            allowed[dominant_genre] = 1.0
            ...
            def get_genre_weight(g_name, base_w):
                ...
                if count == 0:
                    ...
                    return base_w + min(count * 0.005, 0.02)

            sg1 = dom_row.get("similar_genre_1")
            if pd.notna(sg1):
                allowed[sg1] = max(allowed.get(sg1, 0), get_genre_weight(sg1, 0.985))
            sg2 = dom_row.get("similar_genre_2")
            if pd.notna(sg2):
                allowed[sg2] = max(allowed.get(sg2, 0), get_genre_weight(sg2, 0.980))
            sg3 = dom_row.get("similar_genre_3")
            if pd.notna(sg3):
                allowed[sg3] = max(allowed.get(sg3, 0), get_genre_weight(sg3, 0.975))
            candidates["weight"] = candidates["track_genre"].map(allowed).fillna(0.01)
            candidates["weighted_sim"] = (
                candidates["sub_sim"] * candidates["weight"]
            ) + candidates["artist_bonus"]
            valid_candidates = candidates[candidates["weight"] >= 0.01]
            cluster_mix = (
                valid_candidates.sort_values(by="weighted_sim", ascending=False)
                .drop_duplicates(subset=["track_name", "artists"])
                .head(10)
            )
        else:
            candidates["weighted_sim"] = (
                candidates["sub_sim"] + candidates["artist_bonus"]
            )
            cluster_mix = (
                candidates.sort_values(by="weighted_sim", ascending=False)
                .drop_duplicates(subset=["track_name", "artists"])
                .head(10)
            )
    ...
    for dominant_genre, mix_list in genre_mixes.items():
        ...
        final_mix = (
            combined_mix.sort_values(by="weighted_sim", ascending=False)
            .drop_duplicates(subset=["track_name", "artists"])
            .head(10)
        )
        ...
        for _, row in final_mix.iterrows():
            output_lines.append(
                f"  - {row ['track_name']} by {row ['artists']} (Genre: {row ['track_genre']}, Sub-Score: {row ['weighted_sim']:.2f}, Artist Bonus: {row ['artist_bonus']:.3f})"
            )
    output_lines.append("")
    ...
    for artist, count in artist_counts.items():
        if count >= 5:
            artist_query_tracks = query_df[
                query_df["artists"].str.contains(
                    artist, na=False, case=False, regex=False
                )
            ]
            if len(artist_query_tracks) == 0:
                continue
            ...
            artist_candidates = df[
                ~df["track_id"].isin(query_track_ids)
                & df["artists"].str.contains(artist, na=False, case=False, regex=False)
            ].copy()
            if len(artist_candidates) == 0:
                continue
            artist_candidates["sim"] = artist_sims[artist_candidates.index]
            artist_mix = (
                artist_candidates.sort_values(by="sim", ascending=False)
                .drop_duplicates(subset=["track_name", "artists"])
                .head(10)
            )
            if len(artist_mix) > 0:
                ...
                for _, row in artist_mix.iterrows():
                    output_lines.append(
                        f"  - {row ['track_name']} by {row ['artists']} (Genre: {row ['track_genre']}, Sub-Score: {row ['sim']:.2f})"
                    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(query_file):
        ...
        query_tracks = (
            df_temp[df_temp["track_genre"] == "acoustic"].head(5)["track_id"].tolist()
        )
